...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Define schema directly instead of reading file
    const schema = `
...
CREATE INDEX IF NOT EXISTS idx_leaderboard_category ON leaderboard(category, score DESC);
    ...
    // Execute schema - individual statements
    try {
      ...
      await sql`CREATE TABLE IF NOT EXISTS game_stats (
        id SERIAL PRIMARY KEY,
        player_id INTEGER REFERENCES players(id),
        stat_name VARCHAR(50) NOT NULL,
        stat_value INTEGER DEFAULT 0,
      ...
      await sql`CREATE TABLE IF NOT EXISTS battle_pass (
        id SERIAL PRIMARY KEY,
        player_id INTEGER REFERENCES players(id),
        current_tier INTEGER DEFAULT 1,
        current_xp INTEGER DEFAULT 0,
      ...
      await sql`CREATE TABLE IF NOT EXISTS player_weapons (
        id SERIAL PRIMARY KEY,
        player_id INTEGER REFERENCES players(id),
        weapon_type VARCHAR(20) NOT NULL,
        ammo INTEGER DEFAULT 0,
